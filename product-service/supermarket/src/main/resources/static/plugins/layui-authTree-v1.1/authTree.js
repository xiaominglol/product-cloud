        ...
        renderAuth: function (tree, dept, opt) {
            ...
            layui.each(tree, function (index, item) {
                ...
                str += '<div><div class="auth-status" style="display: flex;flex-direction: row;align-items: flex-end;"> ' +
                    (hasChild ? '<i class="layui-icon auth-icon ' + (openstatus ? 'active' : '') + '" style="cursor:pointer;margin-bottom: 3px;margin-left: 10px;">' + (openstatus ? obj.openIconContent : obj.closeIconContent) + '</i>' : '<i class="layui-icon auth-leaf" style="opacity:0;margin-right: 9px;margin-left: 10px">&#xe626;</i>') +
                    (dept > 0 ? '<span>├─ </span>' : '') +
